...
export function AttendanceChart({ data }: { data: { status: AppointmentStatus; count: number }[] }) {
  const chartData = data.map((d) => ({ ...d, label: STATUS_LABEL[d.status], fill: STATUS_COLOR[d.status] }));
